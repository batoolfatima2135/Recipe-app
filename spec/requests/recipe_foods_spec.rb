...
RSpec.describe RecipeFoodsController, type: :request do
  ...
  let(:recipe) { Recipe.create(user: user, name: 'test', description: 'test description', cooking_time: 1, preparation_time: 1) } 
  let(:food) { Food.create(user: user, name: 'Example Food', quantity: 500, measurement_unit: 'gm', price: 1) }
  let(:recipe_food) { RecipeFood.create(quantity: 1, food: food, recipe: recipe) } 
  before(:each) do
    login_as(user, scope: :user)
    Food.create(user: user, name: 'test', quantity: 500, measurement_unit: 'gm', price: 1)
    RecipeFood.create(quantity: 1, food: food, recipe: recipe)
  end
  ...
  describe "POST /recipes/:recipe_id/recipe_foods" do
    ...
    it "creates a new recipe_food" do
      ...
      post recipe_recipe_foods_path(recipe), params: { recipe_food: { food_name: 'test', quantity: 5, recipe_id: recipe.id} }
    ...
    it "fails to create if food doesn't exist" do
      ...
      post recipe_recipe_foods_path(recipe.id), params: { recipe_food: { food_name: "Non-Existent Food", quantity: 5, recipe_id: recipe.id } }
