...
RSpec.feature 'Recipes public', type: :feature do
  ...
  before(:each) do
    ...
    @recipe1 = Recipe.create(name: 'Recipe 1', description: 'Description 1', preparation_time: 1, cooking_time: 2, user:)
    @recipe2 = Recipe.create(name: 'Recipe 2', description: 'Description 2', preparation_time: 2, cooking_time: 3, user:)
    @recipe3 = Recipe.create(name: 'Recipe 3', description: 'Description 3', preparation_time: 1, cooking_time: 1, user:, public: true)
    @recipe4 = Recipe.create(name: 'Recipe 4', description: 'Description 4', preparation_time: 1, cooking_time: 3, user: user2, public: true)
    visit '/'
  end
  ...
  scenario 'User views recipe by click on description' do
      ...
      visit '/'
      click_link(@recipe4.description)
      expect(page).to have_current_path(recipe_path(@recipe4))
  ...
  scenario 'User deletes recipe by button' do
     ...
     expect(page).not_to have_content(@recipe3.description)
     expect{ Recipe.find(@recipe3.id) }.to raise_error(ActiveRecord::RecordNotFound)
